...
create_outcome_table <- function(outcome, ds_cont, ds_cat, design_obj) {
  ...
  for (q in 1:5) {
    ...
    # 3. Number of cases (weighted)
    design_quintile_cases <- subset(design_obj, get(ds_cat) == quintile_label & get(outcome) == 1)
    results$N_Cases_weighted[q] <- sprintf("%.0f", sum(weights(design_quintile_cases)))
  }
  ...
  # Weighted total
  design_total_cases <- subset(design_obj, get(outcome) == 1)
  results$N_Cases_weighted[6] <- sprintf("%.0f", sum(weights(design_total_cases)))
...
for (ds_cat in diet_scores_cat) {
  ...
  p <- ggplot(plot_data, aes(x = Quintile)) +
    # HR line (red)
    geom_line(aes(y = HR), color = "#E31A1C", linewidth = 1) +
    geom_point(aes(y = HR), color = "#E31A1C", size = 3, shape = 16) +
    geom_errorbar(aes(ymin = HR_lower, ymax = HR_upper), 
                  color = "#E31A1C", width = 0.15, linewidth = 0.8) +
    
    # GHG line (blue) - transformed to HR scale
    geom_line(aes(y = GHG * coef_scale + intercept_scale), 
              color = "#1F78B4", linewidth = 1) +
    geom_point(aes(y = GHG * coef_scale + intercept_scale), 
               color = "#1F78B4", size = 3, shape = 16) +
    geom_errorbar(aes(ymin = GHG_lower * coef_scale + intercept_scale,
                      ymax = GHG_upper * coef_scale + intercept_scale), 
                  color = "#1F78B4", width = 0.15, linewidth = 0.8) +
  ...
  # Add legend
  p <- p + 
    annotate("segment", x = 1.1, xend = 1.4, y = min(plot_data$HR_lower), 
             yend = min(plot_data$HR_lower), 
             color = "#E31A1C", linewidth = 1) +
    annotate("point", x = 1.25, y = min(plot_data$HR_lower), 
             color = "#E31A1C", size = 2) +
    annotate("text", x = 1.45, y = min(plot_data$HR_lower), 
             label = "HR", hjust = 0, size = 2.5) +
    
    annotate("segment", x = 2.5, xend = 2.8, y = min(plot_data$HR_lower), 
             yend = min(plot_data$HR_lower), 
             color = "#1F78B4", linewidth = 1) +
    annotate("point", x = 2.65, y = min(plot_data$HR_lower), 
             color = "#1F78B4", size = 2) +
    annotate("text", x = 2.85, y = min(plot_data$HR_lower), 
             label = "GHG", hjust = 0, size = 2.5)
...
diet_survey_summary <- bind_rows(all_diet_summaries) %>%
  mutate(
    diet_score_label = case_when(
      diet_score == "HEI2020_ALL" ~ "HEI-2020",
      diet_score == "AHEI_ALL" ~ "AHEI",
      diet_score == "DASH_ALL" ~ "DASH",
      diet_score == "MED_ALL" ~ "Mediterranean",
      diet_score == "DII_ALL" ~ "DII",
      diet_score == "PHDI_ALL_mean" ~ "PHDI"
    )
  )

# Plot all diet scores with error bars 
diet_trend <- ggplot(diet_survey_summary, aes(x = cycle, y = SE_score, color = diet_score_label, group = diet_score_label)) +
  geom_line(linewidth = 1) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = SE_score - se, ymax = SE_score + se), width = 0.2) +
  theme_bw() +
  labs(x = "NHANES Cycle", y = "Mean Diet Score", color = "Diet Score") +
...
```{r}
#| label: fig-foodgroup-diseases
#| fig-cap: "Multivariable-adjusted associations between food groups and health outcomes (OR comparing 90th to 10th percentile)"
#| fig-width: 12
#| fig-height: 8
...
for (i in 1:length(food_groups)) {
  for (j in 1:length(outcomes)) {
    ...
    # Fit logistic regression adjusted for age
    formula_model <- as.formula(paste0(outcome, " ~ ", fg, " + RIDAGEYR"))
...
# Create heatmap with green (OR > 1) and pink (OR < 1) colors
ggplot(or_df, aes(x = Outcome, y = Food_Group, fill = OR)) +
  ...
  scale_fill_gradient2(
    low = "#E91E63",     # Pink for OR < 1 (protective)
    mid = "white", 
    high = "#4CAF50",    # Green for OR > 1 (risk)
    midpoint = 1,
    limits = c(0.1, 2.0),
    na.value = "grey90",
    name = "Odds Ratio\n(90th vs 10th\npercentile)"
