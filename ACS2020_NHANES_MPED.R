#' ACS2020_NHANES_MPED
#'
#' Calculate the American Cancer Society 2020 dietary index (ACS2020) for the NHANES_MPED data (before 2005, 1999-2004) within 1 step for day 1, day 2, or day 1 and 2 combined (age >= 2 only)
#' @import dplyr
#' @import readr
#' @import haven
...
#' ACS2020_NHANES_MPED(MPED_PER_100_GRAM_PATH = NHANES_20032004$MPED_PER_100_GRAM, WJFRT = NHANES_20032004$WJFRT, NUTRIENT_PATH = NHANES_20032004$NUTRIENT, NUTRIENT_IND_PATH = NHANES_20032004$NUTRIENT_IND, DEMO_PATH = NHANES_20032004$DEMO, NUTRIENT_PATH2 = NHANES_20032004$NUTRIENT2, NUTRIENT_IND_PATH2 = NHANES_20032004$NUTRIENT_IND2)
#' @export

ACS2020_NHANES_MPED = function(MPED_PER_100_GRAM_PATH = NULL, WJFRT = NULL, NUTRIENT_PATH = NULL, NUTRIENT_IND_PATH = NULL, DEMO_PATH, NUTRIENT_PATH2 = NULL, NUTRIENT_IND_PATH2 = NULL, SSB_code = NULL) {
    ...
    if (("ModCode" %in% colnames(WJFRT)) == TRUE | ("MODCODE" %in% colnames(WJFRT)) == TRUE) {
    ...
    if (!is.null(NUTRIENT_PATH) & !is.null(NUTRIENT_IND_PATH)) {
        ...
        # calculate the sum of each food group for each individual
        MPED_ITEMS_SERV <- MPED_IND_2 %>%
            group_by(SEQN) %>%
            summarise(
                VEG_ITEMS_SERV = length(unique(DR1IFDCD[V_DRKGR > 0 | V_ORANGE > 0 | V_OTHER > 0 | V_TOMATO > 0 | V_STARCY > 0])),
                FRT_ITEMS_SERV = length(unique(DR1IFDCD[WHOLEFRT > 0]))
            )

        # combine MPED and MPED_ITEMS_SERV
        MPED = left_join(MPED, MPED_ITEMS_SERV, by = "SEQN")
        ...
        # calculate the ACS2020 food group serving size / 1000 kcal
        COHORT_3 = COHORT_2 %>%
            dplyr::mutate(
                VEG_SERV = V_DRKGR + ((V_ORANGE + V_TOMATO + V_OTHER + V_STARCY) / 0.5),
                VEG_ITEMS_SERV = VEG_ITEMS_SERV,
                FRT_SERV = WHOLEFRT,
                FRT_ITEMS_SERV = FRT_ITEMS_SERV,
                ...
                HPFRG_SERV = G_NWHL / 0.035274
            )
        
        # select those who have more than 0 kcal intake
        COHORT_3 = COHORT_3 %>%
            filter(DR1TKCAL > 0)

        # use the ACS2020 generic function to calculate the ACS2020 total and component scores
        COHORT_summary_ACS2020 = ACS2020_V2(
            SERV_DATA = COHORT_3,
            RESPONDENTID = COHORT_3$SEQN,
            GENDER = COHORT_3$RIAGENDR,
            TOTALKCAL_ACS2020 = COHORT_3$DR1TKCAL,
            VEG_SERV_ACS2020 = COHORT_3$VEG_SERV,
            VEG_ITEMS_SERV_ACS2020 = COHORT_3$VEG_ITEMS_SERV,
            FRT_SERV_ACS2020 = COHORT_3$FRT_SERV,
            FRT_ITEMS_SERV_ACS2020 = COHORT_3$FRT_ITEMS_SERV,
            WGRAIN_SERV_ACS2020 = COHORT_3$WGRAIN_SERV,
            REDPROC_MEAT_SERV_ACS2020 = COHORT_3$REDPROC_MEAT_SERV,
            HPFRG_SERV_ACS2020 = COHORT_3$HPFRG_SERV,
            SSB_FRTJ_SERV_ACS2020 = COHORT_3$SSB_FRTJ_SERV
        )
    ...
    if (!is.null(NUTRIENT_PATH2) & !is.null(NUTRIENT_IND_PATH2)) {
        ...
        # calculate the sum of each food group for each individual
        MPED_ITEMS_SERV2 <- MPED_PER_100_GRAM2_5 %>%
            group_by(SEQN) %>%
            summarise(
                VEG_ITEMS_SERV = length(unique(DR2IFDCD[V_DRKGR > 0 | V_ORANGE > 0 | V_OTHER > 0 | V_TOMATO > 0 | V_STARCY > 0])),
                FRT_ITEMS_SERV = length(unique(DR2IFDCD[WHOLEFRT > 0]))
            )

        # combine MPED2 and MPED_ITEMS_SERV2
        MPED2 = left_join(MPED2, MPED_ITEMS_SERV2, by = "SEQN")
        ...
        COHORT2_summary_ACS2020 = ACS2020_V2(
            ...
            TOTALKCAL_ACS2020 = COHORT2_3$DR2TKCAL,
            VEG_SERV_ACS2020 = COHORT2_3$VEG_SERV,
            VEG_ITEMS_SERV_ACS2020 = COHORT2_3$VEG_ITEMS_SERV,
            FRT_SERV_ACS2020 = COHORT2_3$FRT_SERV,
            FRT_ITEMS_SERV_ACS2020 = COHORT2_3$FRT_ITEMS_SERV,
            WGRAIN_SERV_ACS2020 = COHORT2_3$WGRAIN_SERV,
            REDPROC_MEAT_SERV_ACS2020 = COHORT2_3$REDPROC_MEAT_SERV,
            HPFRG_SERV_ACS2020 = COHORT2_3$HPFRG_SERV,
            SSB_FRTJ_SERV_ACS2020 = COHORT2_3$SSB_FRTJ_SERV
        )
