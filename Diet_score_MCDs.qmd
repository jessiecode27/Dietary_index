...
setwd("/Users/jessie/Desktop/Vyy/Projects 2025/Dietscorepaper_Dr.Linh")
library(dplyr)
...
years   <- c("2005","2007","2009","2011","2013","2015","2017") #start year of the cycle
suffix  <- c("_D","_E","_F","_G","_H","_I","_P") # _P for 2017-Mar2020
cycles  <- c("0506","0708","0910","1112","1314","1516","1720")
cycles_0518  <- c("0506","0708","0910","1112","1314","1516","1718")
letters <- c("D","E","F","G","H","I","P")
```
...
# Groups of variables
demo_cols   <- c("RIAGENDR","RIDAGEYR","SDMVSTRA","SDMVPSU","DMDEDUC2",
                 "WTMEC2YR","WTINT2YR",        # 2-year weights (2005-2016)
                 "WTMECPRP","WTINTPRP",        # 4-year pre-pandemic weights (2017–Mar 2020)
                 
                 "RIDRETH1","DMDMARTL","INDFMPIR",
                 "DMDHHSIZ")                   # household size
mcq_cols    <- c("MCQ160B","MCQ160C","MCQ160D","MCQ160E","MCQ160F","MCQ220")
diq_cols    <- c("DIQ010","DIQ050", "DID060", "DIQ070", "DID040", "DIQ175S")
bpq_cols    <- c("BPQ020")

bpx_cols    <- c("BPXSY1","BPXSY2","BPXSY3","BPXSY4","BPXDI1","BPXDI2","BPXDI3","BPXDI4") # 2005-2016 cycles have 4 readings
bpxo_cols   <- c("BPXOSY1","BPXOSY2","BPXOSY3","BPXODI1","BPXODI2","BPXODI3") # 2017-Mar2020 cycle have 3 readings
...
for (i in seq_along(years)) {
  ...
  U <- function(stub) {
    if (yr == "2017") {
    # 2017–March 2020: file có prefix "P_"
    sprintf("https://wwwn.cdc.gov/Nchs/Data/Nhanes/Public/2017/DataFiles/P_%s.XPT", stub)
    } else {
    sprintf("https://wwwn.cdc.gov/Nchs/Data/Nhanes/Public/%s/DataFiles/%s%s.XPT", 
            yr, stub, suf)
    }
  }
  # Read each component
...
demo_health_0520 <- bind_rows(per_cycle, .id = "cycle")
glimpse(demo_health_0520)
# View(demo_health_0518)

# supplement "vitamin/mineral supplement use" (DSD010) variable
dsd_table <- c("DSQ1_D", "DSQTOT_E", "DSQTOT_F", "DSQTOT_G", "DSQTOT_H", "DSQTOT_I", "P_DSQTOT")
...
dsd_0520 <- bind_rows(dsd_list)
dsd_0520 <- select(dsd_0520, c("SEQN", "DSD010"))

# merge dsd in "demo_health_0518" df
demo_health_0518 <- demo_health_0520 %>%
  left_join(dsd_0520, by = "SEQN")

# Save demo_health_0518 to CSV file
write_csv(demo_health_0520, "DATA/demo_health_0520.csv")

# Clear from memory and load back from CSV
rm(demo_health_0520, dsd_0520, dsd_list)
gc()
```

```{r load demo_health_0520}
# Load demo_health_0520 back from CSV
demo_health_0520 <- read_csv("DATA/demo_health_0520.csv", show_col_types = FALSE)
```
...
for (i in seq_along(cycles)) {
  ...
  if (cyc == "1720") {
    cycle_list <- list(
      DEMO          = read_xpt(file.path(demo_dir, "P_DEMO.XPT")),
      
      NUTRIENT      = read_xpt(file.path(nutr_dir, "P_DR1TOT.xpt")),
      NUTRIENT2     = read_xpt(file.path(nutr_dir, "P_DR2TOT.xpt")),
      NUTRIENT_IND  = read_xpt(file.path(nutr_dir, "P_DR1IFF.xpt")),
      NUTRIENT_IND2 = read_xpt(file.path(nutr_dir, "P_DR2IFF.xpt")),
      
      FPED          = read_sas(file.path(fped_dir, "fped_dr1tot_1720.sas7bdat")),
      FPED2         = read_sas(file.path(fped_dir, "fped_dr2tot_1720.sas7bdat")),
      FPED_IND      = read_sas(file.path(fped_dir, "fped_dr1iff_1720.sas7bdat")),
      FPED_IND2     = read_sas(file.path(fped_dir, "fped_dr2iff_1720.sas7bdat"))
    )
  ...
  # Save this cycle into the outer list, named by its code (e.g., "0506")
...
# Prepare HEI2020 specific data
HEI2020_0520 <- map_dfr(cycles, function(cyc) { 
  dat <- NHANES_ALL[[cyc]]
...
AHEI_0520 <- map_dfr(cycles, function(cyc) {
  dat <- NHANES_ALL[[cyc]]
...
DASH_0520 <- map_dfr(cycles, function(cyc) {
  dat <- NHANES_ALL[[cyc]]
...
AMED_0520 <- map_dfr(cycles, function(cyc) {
  dat <- NHANES_ALL[[cyc]]
  out <- MED_NHANES_FPED(
    FPED_PATH      = dat$FPED,
    NUTRIENT_PATH  = dat$NUTRIENT,
...
DII_0520 <- map_dfr(cycles, function(cyc) {
  dat <- NHANES_ALL[[cyc]]
  out <- DII_NHANES_FPED(
    FPED_PATH      = dat$FPED,
    NUTRIENT_PATH  = dat$NUTRIENT,
...
ACS2020_0520 <- map_dfr(cycles, function(cyc) {
  dat <- NHANES_ALL[[cyc]]
...
base_path <- "/Users/jessie/Desktop/Vyy/Projects 2025/Dietscorepaper_Dr.Linh/CODE/R script/PHDI and GHG calculation/PHDI calculation/Step 6. Create FPED files for individual foods and each individual /Output/FPED"

## 2017-2018
FPED_DR1TOT_gram_1718_d1 = read_csv(file.path(base_path, "FPED_DR1TOT_gram_1718_d1.csv"))
FPED_DR1TOT_gram_1718_d2 = read_csv(file.path(base_path, "FPED_DR1TOT_gram_1718_d2.csv"))

## 2015-2016
FPED_DR1TOT_gram_1516_d1 = read_csv(file.path(base_path, "FPED_DR1TOT_gram_1516_d1.csv"))
FPED_DR1TOT_gram_1516_d2 = read_csv(file.path(base_path, "FPED_DR1TOT_gram_1516_d2.csv"))

## 2013-2014
FPED_DR1TOT_gram_1314_d1 = read_csv(file.path(base_path, "FPED_DR1TOT_gram_1314_d1.csv"))
FPED_DR1TOT_gram_1314_d2 = read_csv(file.path(base_path, "FPED_DR1TOT_gram_1314_d2.csv"))

## 2011-2012
FPED_DR1TOT_gram_1112_d1 = read_csv(file.path(base_path, "FPED_DR1TOT_gram_1112_d1.csv"))
FPED_DR1TOT_gram_1112_d2 = read_csv(file.path(base_path, "FPED_DR1TOT_gram_1112_d2.csv"))

## 2009-2010
FPED_DR1TOT_gram_0910_d1 = read_csv(file.path(base_path, "FPED_DR1TOT_gram_0910_d1.csv"))
FPED_DR1TOT_gram_0910_d2 = read_csv(file.path(base_path, "FPED_DR1TOT_gram_0910_d2.csv"))

## 2007-2008
FPED_DR1TOT_gram_0708_d1 = read_csv(file.path(base_path, "FPED_DR1TOT_gram_0708_d1.csv"))
FPED_DR1TOT_gram_0708_d2 = read_csv(file.path(base_path, "FPED_DR1TOT_gram_0708_d2.csv"))

## 2005-2006
FPED_DR1TOT_gram_0506_d1 = read_csv(file.path(base_path, "FPED_DR1TOT_gram_0506_d1.csv"))
FPED_DR1TOT_gram_0506_d2 = read_csv(file.path(base_path, "FPED_DR1TOT_gram_0506_d2.csv"))
...
for (cyc in cycles) {
  ...
  # Trong NHANES_ALL cycle chỉ có 1720, còn cycle trong tên file FPED là 1718
  fcyc <- if (cyc == "1720") "1718" else cyc

  # lấy dữ liệu FPED tương ứng theo tên biến
  fped_d1 <- get(paste0("FPED_DR1TOT_gram_", fcyc, "_d1"))
  fped_d2 <- get(paste0("FPED_DR1TOT_gram_", fcyc, "_d2"))
...
# tibble 
FPED_NUTRIENT_DEMO_list_d1[["1720"]] %>% glimpse()
...
#include SEQN cho cả 0520 nhưng điểm PHDI chỉ được tính cho 0518, còn 1920 NA
```
...
cyc_years <- c(
  ...
  "1314" = "2013_2014",
  "1516" = "2015_2016",
  "1720" = "2017_2018" # 1720 để khớp với biến cycle chung
)
...
MORT_DIR <- "/Users/jessie/Desktop/Vyy/Projects 2025/Dietscorepaper_Dr.Linh/DATA/Mortality"
...
BASE <- "/Users/jessie/Desktop/Vyy/Projects 2025/Dietscorepaper_Dr.Linh/CODE/R script/PHDI and GHG calculation/GHG calculation/Step 2/Input/GHG"
...
for (cyc in cycles) {
  # để khớp với biến cycle chung 
  fcyc <- if (cyc == "1720") "1718" else cyc
  # build file paths
  d1_path <- file.path (BASE, paste0("GHG_DR1TOT_gram_", fcyc, ".csv"))
  d2_path <- file.path (BASE, paste0("GHG_DR2TOT_gram_", fcyc, ".csv"))
...
glimpse(nhanes_merged_0520)
#View(nhanes_merged_0518)

# Save nhanes_merged_0518 to CSV file
write_csv(nhanes_merged_0520, "DATA/nhanes_merged_0520.csv")
```

```{r load nhanes_merged_0518}
# Load nhanes_merged_0518 back from CSV
nhanes_merged_0518 <- read_csv("DATA/nhanes_merged_0518.csv", show_col_types = FALSE)
```
