...
demo_health_0518 <- read_csv("DATA/demo_health_0518.csv", show_col_types = FALSE)
```
...
BASE <- "../CODE/Diet score/DATA"

# Define cycles and their NHANES letter suffixes 
cycles  <- c("0506","0708","0910","1112","1314","1516","1718")
letters <- c("D","E","F","G","H","I","J")

# Function to read dietary data for specific dietary indices
read_dietary_data <- function(cyc, let, data_types) {
  ...
  # Read data based on required types
  if ("DEMO" %in% data_types) {
    cycle_data$DEMO <- read_xpt(file.path(demo_dir, sprintf("DEMO_%s.xpt", let)))
  }
  if ("FPED" %in% data_types) {
    cycle_data$FPED <- read_sas(file.path(fped_dir, sprintf("fped_dr1tot_%s.sas7bdat", cyc)))
  }
  if ("NUTRIENT" %in% data_types) {
    cycle_data$NUTRIENT <- read_xpt(file.path(nutr_dir, sprintf("DR1TOT_%s.xpt", let)))
  }
  if ("FPED2" %in% data_types) {
    cycle_data$FPED2 <- read_sas(file.path(fped_dir, sprintf("fped_dr2tot_%s.sas7bdat", cyc)))
  }
  if ("NUTRIENT2" %in% data_types) {
    cycle_data$NUTRIENT2 <- read_xpt(file.path(nutr_dir, sprintf("DR2TOT_%s.xpt", let)))
  }
  if ("NUTRIENT_IND" %in% data_types) {
    cycle_data$NUTRIENT_IND <- read_xpt(file.path(nutr_dir, sprintf("DR1IFF_%s.xpt", let)))
  }
  if ("FPED_IND" %in% data_types) {
    cycle_data$FPED_IND <- read_sas(file.path(fped_dir, sprintf("fped_dr1iff_%s.sas7bdat", cyc)))
  }
  if ("FPED_IND2" %in% data_types) {
    cycle_data$FPED_IND2 <- read_sas(file.path(fped_dir, sprintf("fped_dr2iff_%s.sas7bdat", cyc)))
  }
  if ("NUTRIENT_IND2" %in% data_types) {
    cycle_data$NUTRIENT_IND2 <- read_xpt(file.path(nutr_dir, sprintf("DR2IFF_%s.xpt", let)))
  }
...
HEI2020_data <- list()
required_files_HEI <- c("DEMO", "FPED", "NUTRIENT", "FPED2", "NUTRIENT2")

for (i in seq_along(cycles)) {
  cyc <- cycles[i]
  let <- letters[i]
  HEI2020_data[[cyc]] <- read_dietary_data(cyc, let, required_files_HEI)
}

# Calculate HEI2020
HEI2020_0518 <- map_dfr(cycles, function(cyc) { 
  dat <- HEI2020_data[[cyc]]
...
# Clear HEI2020 data to save memory
rm(HEI2020_data)
gc()
...
AHEI_data <- list()
required_files_AHEI <- c("FPED_IND", "NUTRIENT_IND", "FPED_IND2", "NUTRIENT_IND2")

for (i in seq_along(cycles)) {
  cyc <- cycles[i]
  let <- letters[i]
  AHEI_data[[cyc]] <- read_dietary_data(cyc, let, required_files_AHEI)
}
...
# Clear AHEI data to save memory
rm(AHEI_data)
gc()
...
DASH_data <- list()
required_files_DASH <- c("FPED_IND", "NUTRIENT_IND", "FPED_IND2", "NUTRIENT_IND2")

for (i in seq_along(cycles)) {
  cyc <- cycles[i]
  let <- letters[i]
  DASH_data[[cyc]] <- read_dietary_data(cyc, let, required_files_DASH)
}
...
# Clear DASH data to save memory
rm(DASH_data)
gc()
...
AMED_data <- list()
required_files_AMED <- c("DEMO", "FPED", "NUTRIENT", "FPED2", "NUTRIENT2")

for (i in seq_along(cycles)) {
  cyc <- cycles[i]
  let <- letters[i]
  AMED_data[[cyc]] <- read_dietary_data(cyc, let, required_files_AMED)
}
...
# Clear aMED data to save memory
rm(AMED_data)
gc()
...
DII_data <- list()
required_files_DII <- c("DEMO", "FPED", "NUTRIENT", "FPED2", "NUTRIENT2")

for (i in seq_along(cycles)) {
  cyc <- cycles[i]
  let <- letters[i]
  DII_data[[cyc]] <- read_dietary_data(cyc, let, required_files_DII)
}
...
# Prepare PHDI specific data (requires DEMO + NUTRIENT for demographics and calories)
PHDI_data <- list()
required_files_PHDI <- c("DEMO", "NUTRIENT", "NUTRIENT2")

for (i in seq_along(cycles)) {
  cyc <- cycles[i]
  let <- letters[i]
  PHDI_data[[cyc]] <- read_dietary_data(cyc, let, required_files_PHDI)
}
...
## Merge nutrient and demographic data for PHDI
...
for (cyc in cycles) {
  ...
  df_merged <- reduce(                            # merge the tables in the list one by one
    list(
    df$DEMO,
    df$NUTRIENT %>% select(SEQN, DR1TKCAL, DR1DRSTZ),   #double-check DR1DRSTZ/DR2DRSTZ
...
rm(PHDI_data, NUTRIENT_DEMO_list)
gc()
...
# Clear PHDI temporary data to save memory
rm(FPED_NUTRIENT_DEMO_list_d1, FPED_NUTRIENT_DEMO_list_d2, PHDI_0518_d1, PHDI_0518_d2, PHDI_0518_d1_d2)
# Clear all FPED gram files
rm(list = ls(pattern = "FPED_DR1TOT_gram"))
gc()
